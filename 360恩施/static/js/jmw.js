        ...
        if (word_cookie == undefined || word_cookie != tj.word) {
            if (tj.word.length != 4) {
                $.ajax({
                    type: "POST",
                    url: "/tools/lp.ashx",
                    data: { action: "get_word_now", ip: _ip, word: tj.word, area: JmwInitJson.model_city, remark: tj.engine, province: _province, city: _city, gid: JmwInitJson.model_lx_gid, kf_name: kefu_data.xingming, kf_id: 0, browser: tj.model, wx: kefu_data.weixinhao, phone: kefu_data.shouji, url: tj.sourceURL, keywordCode: tj.keywordCode, urlreferrer: url, keyword: tj.keyword, width: window.innerWidth, height: window.innerHeight, adposition: tj.adposition },
                    dataType: "json",
                    jsonp: "callback",
                    success: function (data) {
                        if (data.status == 1) {//成功
...
function JmwLlsj() {
    if (id_new > 0) {
        $.ajax({
            type: "POST",
            url: "/tools/lp.ashx",
            data: { action: "browse", urlr_id: id_new, wl_id: wl_id, browse_time: browse_time, mcount: mcount, playback_data: playback_data, touch_data: touch_data, operation_data: operation_data },
            dataType: "json",
            jsonp: "callback",
            success: function (data) {
                if (data.status == 1) {
